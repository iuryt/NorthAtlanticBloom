...
popt_argo, pcov_argo = curve_fit(func, argo.PRES_INTERPOLATED.values, argo.PDEN.values,p0=[0.2,6e-3,500,0,1027.6])
argo = argo.assign(PDENf=("PRES_INTERPOLATED",func(argo.PRES_INTERPOLATED.values,*popt_argo)))
...
_ = (argo.PDEN-1000)[::100].plot.line(ax=ax["pden"], y="PRES_INTERPOLATED", ylim=[1000,0], lw=0, marker="o", color=colors["obs"])
_ = (argo.PDENf-1000).plot.line(ax=ax["pden"], y="PRES_INTERPOLATED", ylim=[1000,0], color=colors["model"])
...
zi = -np.arange(1000)
ax["chla"].plot(0.1*(np.tanh(1e-2*(zi+300))+1)/2, zi, color=colors["model"])
ax["no3"].plot(zi*(12-16)/(0+800)+12, zi, color=colors["model"])
